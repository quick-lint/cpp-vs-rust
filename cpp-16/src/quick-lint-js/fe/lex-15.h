...
// A lexer_15 reads JavaScript source code one token at a time.
//
...
//
// lexer_15 can modify the input string in some cases. For example, the identifier
...
class lexer_15 {
 public:
  ...
  explicit lexer_15(padded_string_view input, diag_reporter*) noexcept;
  ...
  lexer_transaction_15 begin_transaction();

  // After calling commit_transaction, it's almost as if you never called
  // begin_transaction in the first place.
  //
  // commit_transaction does not restore the state of the lexer_15 when
  // begin_transaction was called.
  void commit_transaction(lexer_transaction_15&&);

  // Restore lexer_15 state to a prior version.
  //
  ...
  // etc.
  //
  // Calling roll_back_transaction will not report lexer_15 diagnostics which might
  // have been reported if it weren't for begin_transaction.
  void roll_back_transaction(lexer_transaction_15&&);

  // transaction_has_lex_diagnostics can only be called while the given
  // transaction is the most recent active transaction.
  bool transaction_has_lex_diagnostics(const lexer_transaction_15&) const noexcept;
 ...
 private:
  ...
  monotonic_allocator allocator_{"lexer_15::allocator_"};
  linked_bump_allocator<alignof(void*)> transaction_allocator_{
      "lexer_15::transaction_allocator_"};
};

struct lexer_transaction_15 {
  // Private to lexer_15. Do not construct, read, or modify.

  using allocator_type = linked_bump_allocator<alignof(void*)>;

  explicit lexer_transaction_15(token old_last_token,
  ...
  // Don't allow copying a transaction. lexer_15::diag_reporter_ might point to
  // lexer_transaction_15::diag_reporter.
  lexer_transaction_15(const lexer_transaction_15&) = delete;
  lexer_transaction_15& operator=(const lexer_transaction_15&) = delete;
