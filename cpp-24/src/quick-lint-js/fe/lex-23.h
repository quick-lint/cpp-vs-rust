...
// A lexer_23 reads JavaScript source code one token at a time.
//
...
//
// lexer_23 can modify the input string in some cases. For example, the identifier
...
class lexer_23 {
 public:
  ...
  explicit lexer_23(padded_string_view input, diag_reporter*) noexcept;
  ...
  lexer_transaction_23 begin_transaction();

  // After calling commit_transaction, it's almost as if you never called
  // begin_transaction in the first place.
  //
  // commit_transaction does not restore the state of the lexer_23 when
  // begin_transaction was called.
  void commit_transaction(lexer_transaction_23&&);

  // Restore lexer_23 state to a prior version.
  //
  ...
  // etc.
  //
  // Calling roll_back_transaction will not report lexer_23 diagnostics which might
  // have been reported if it weren't for begin_transaction.
  void roll_back_transaction(lexer_transaction_23&&);

  // transaction_has_lex_diagnostics can only be called while the given
  // transaction is the most recent active transaction.
  bool transaction_has_lex_diagnostics(const lexer_transaction_23&) const noexcept;
 ...
 private:
  ...
  monotonic_allocator allocator_{"lexer_23::allocator_"};
  linked_bump_allocator<alignof(void*)> transaction_allocator_{
      "lexer_23::transaction_allocator_"};
};

struct lexer_transaction_23 {
  // Private to lexer_23. Do not construct, read, or modify.

  using allocator_type = linked_bump_allocator<alignof(void*)>;

  explicit lexer_transaction_23(token old_last_token,
  ...
  // Don't allow copying a transaction. lexer_23::diag_reporter_ might point to
  // lexer_transaction_23::diag_reporter.
  lexer_transaction_23(const lexer_transaction_23&) = delete;
  lexer_transaction_23& operator=(const lexer_transaction_23&) = delete;
