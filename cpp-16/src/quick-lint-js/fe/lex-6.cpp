...
lexer_6::lexer_6(padded_string_view input, diag_reporter* diag_reporter) noexcept
    : input_(input.data()),
...
void lexer_6::parse_bom_before_shebang() {
...
[[gnu::noinline]] void lexer_6::parse_current_token() {
...
bool lexer_6::try_parse_current_token() {
...
bool lexer_6::test_for_regexp(const char8* regexp_begin) {
  lexer_transaction_6 transaction = this->begin_transaction();
...
const char8* lexer_6::parse_string_literal() noexcept {
...
const char8* lexer_6::parse_jsx_string_literal() noexcept {
  char8 opening_quote = this->input_[0];
  const char8* c = &this->input_[1];
...
const char8* lexer_6::parse_smart_quote_string_literal(
...
void lexer_6::skip_in_template(const char8* template_begin) {
...
lexer_6::parsed_template_body lexer_6::parse_template_body(
...
void lexer_6::skip_in_jsx_children() {
  QLJS_ASSERT(this->peek().type == token_type::right_curly ||
...
const char8* lexer_6::find_equal_greater_in_jsx_children() const noexcept {
  const char8* c;
  for (c = this->input_;; ++c) {
    switch (*c) {
    case u8'<':
...
void lexer_6::skip_less_less_as_less() {
...
void lexer_6::reparse_as_regexp() {
...
lexer_transaction_6 lexer_6::begin_transaction() {
  return lexer_transaction_6(
...
void lexer_6::commit_transaction(lexer_transaction_6&& transaction) {
...
void lexer_6::roll_back_transaction(lexer_transaction_6&& transaction) {
...
bool lexer_6::transaction_has_lex_diagnostics(const lexer_transaction_6&) const
    noexcept {
  buffering_diag_reporter* buffered_diagnostics =
      static_cast<buffering_diag_reporter*>(this->diag_reporter_);
...
const char8* lexer_6::end_of_previous_token() const noexcept {
...
padded_string_view lexer_6::original_input() const noexcept {
  return this->original_input_;
}

template <class Error>
const char8* lexer_6::check_garbage_in_number_literal(const char8* input) {
...
void lexer_6::parse_binary_number() {
  const char8* input = this->input_;
...
void lexer_6::parse_legacy_octal_number() {
  const char8* input = this->input_;
...
void lexer_6::parse_modern_octal_number() {
...
void lexer_6::check_integer_precision_loss(string8_view number_literal) {
...
void lexer_6::parse_hexadecimal_number() {
  const char8* input = this->input_;
...
template <class Func>
const char8* lexer_6::parse_digits_and_underscores(Func&& is_valid_digit,
...
const char8* lexer_6::parse_octal_digits(const char8* input) noexcept {
...
const char8* lexer_6::parse_decimal_digits_and_underscores(
    const char8* input) noexcept {
  return this->parse_digits_and_underscores(
      [](char8 character) -> bool { return is_digit(character); }, input);
}

const char8* lexer_6::parse_hex_digits_and_underscores(
    const char8* input) noexcept {
  return this->parse_digits_and_underscores(
...
QLJS_WARNING_PUSH
QLJS_WARNING_IGNORE_GCC("-Wuseless-cast")
lexer_6::parsed_unicode_escape lexer_6::parse_unicode_escape(
...
lexer_6::parsed_identifier lexer_6::parse_identifier(const char8* input,
...
const char8* lexer_6::parse_identifier_fast_only(const char8* input) {
  // TODO(strager): Is the check for '\\' correct?
...
QLJS_WARNING_PUSH
QLJS_WARNING_IGNORE_GCC("-Wuseless-cast")
lexer_6::parsed_identifier lexer_6::parse_identifier_slow(
...
void lexer_6::skip_block_comment() {
...
void lexer_6::skip_line_comment_body() {
#if QLJS_HAVE_ARM_NEON
  using bool_vector = bool_vector_16_neon;
  using char_vector = char_vector_16_neon;
#elif QLJS_HAVE_WEB_ASSEMBLY_SIMD128
...
bool lexer_6::is_eof(const char8* input) const noexcept {
  QLJS_ASSERT(*input == u8'\0');
  return input == this->original_input_.null_terminator();
}

bool lexer_6::is_first_token_on_line() const noexcept {
  return this->last_token_.has_leading_newline ||
         this->last_last_token_end_ == this->original_input_.data();
}

bool lexer_6::is_binary_digit(char8 c) { return c == u8'0' || c == u8'1'; }

bool lexer_6::is_octal_digit(char8 c) {
  switch (c) {
  QLJS_CASE_OCTAL_DIGIT:
    return true;
  default:
...
bool lexer_6::is_hex_digit(char8 c) {
  switch (c) {
  QLJS_CASE_DECIMAL_DIGIT:
  case 'a':
  case 'b':
...
bool lexer_6::is_initial_identifier_byte(char8 byte) {
...
bool lexer_6::is_identifier_byte(char8 byte) {
  switch (static_cast<std::uint8_t>(byte)) {
  QLJS_CASE_DECIMAL_DIGIT:
  QLJS_CASE_IDENTIFIER_START:
  // clang-format off
...
bool lexer_6::is_initial_identifier_character(char32_t code_point) {
  return look_up_in_unicode_table(identifier_start_chunk_indexes,
                                  identifier_start_chunk_indexes_size,
                                  code_point);
}

bool lexer_6::is_identifier_character(char32_t code_point, identifier_kind kind) {
...
bool lexer_6::is_non_ascii_whitespace_character(char32_t code_point) {
...
bool lexer_6::is_ascii_character(char8 code_unit) {
  return static_cast<unsigned char>(code_unit) < 0x80;
}

bool lexer_6::is_ascii_character(char32_t code_point) {
  return code_point < 0x80;
}

int lexer_6::newline_character_size(const char8* input) {
  if (input[0] == u8'\n' || input[0] == u8'\r') {
    return 1;
  }
...
bool lexer_6::is_newline_character(char32_t code_point) noexcept {
  return code_point == U'\n' || code_point == U'\r' ||
