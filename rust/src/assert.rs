...
macro_rules! qljs_always_assert {
    ($cond:expr $(,)?) => {
        if !$cond {
            $crate::assert::report_assertion_failure(
...
// function. Perhaps we should look at other solutions.
pub fn report_assertion_failure(file_name: *const u8, line: u32, message: *const u8) {
    unsafe {
